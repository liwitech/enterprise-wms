...
function MilestoneList({ milestones }: { milestones: { id: string; title: string; due_date?: string; status: string }[] }) {
  ...
  if (!project) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 text-slate-500">
        ...
        <button onClick={() => navigate('/projects')} className="text-sm text-indigo-600 hover:underline">
          Quay lại danh sách
        </button>
      </div>
    )
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="shrink-0 border-b border-slate-200 bg-white px-6 py-4">
        <button
          ...
          className="mb-3 flex items-center gap-1.5 text-xs text-slate-500 hover:text-indigo-600 transition-colors"
        >
